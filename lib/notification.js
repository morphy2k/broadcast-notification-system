...
module.exports = () => {
	...
	if (length < 5) {
		...
		if (db.queue.get('follows.list').size().value() !== 0) {
			...
			queue.push(obj);
			db.queue.get('follows.list').remove(element).value();
		}
		// subscriptions
		if (db.queue.get('subs.list').size().value() !== 0) {
			...
			queue.push(obj);
			db.queue.get('subs.list').remove(element).value();
		}
		// hosts
		if (db.queue.get('hosts.list').size().value() !== 0) {
			...
			queue.push(obj);
			db.queue.get('hosts.list').remove(element).value();
		}
		// donations
		if (db.queue.get('donations.list').size().value() !== 0) {
			...
			queue.push(obj);
			db.queue.get('donations.list').remove(element).value();
		}
