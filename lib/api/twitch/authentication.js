    ...
    new Promise((resolve, reject) => {
      ...
      request.post('https://api.twitch.tv/kraken/oauth2/token',
        post, (err, response, body) => {
          if (err) {
            reject(new Error(`[TWITCH AUTH]: S1 ${err}`));
            ...
            reject(new Error(`[TWITCH AUTH]: S1 HTTP ${response.statusCode}`));
          }
      ...
      return new Promise((resolve, reject) => {
        ...
        request(`https://api.twitch.tv/kraken?oauth_token=${token}`,
          this.options, (err, response, data) => {
            if (err) {
              reject(new Error(`[TWITCH AUTH]: S2 ${err}`));
              ...
              reject(new Error(`[TWITCH AUTH]: S2 HTTP ${response.statusCode}`));
            }
          });
      ...
      return new Promise((resolve, reject) => {
        ...
        request(`https://api.twitch.tv/kraken/channels/${data.name}`,
          this.options, (err, response, data) => {
            if (err) {
              reject(new Error(`[TWITCH AUTH]: S3 ${err}`));
              ...
              reject(new Error(`[TWITCH AUTH]: S3 HTTP ${response.statusCode}`));
            }
          });
      });
    }).then(user => {
      ...
      require('./').init();
      return require('../../scheduler').api('twitch');
