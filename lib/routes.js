...
app.use(async(ctx, next) => {
    ...
    if (ctx.method === 'GET') {
        ...
        } else if (ctx.url.startsWith('/authentication/signin/send?email')) {
            if (!email && ctx.request.query.email) {
                email = ctx.request.query.email;
                authentication.signIn(email);
                ctx.body = `Please check your mailbox in 1-2 minutes!`;
            } else {
                ctx.throw(403);
            }
        }
...
app.use(async(ctx, next) => {
    ...
    if (ctx.method === 'GET') {
        ...
        if (ctx.url === '/notification') {
            ...
            if (page.length > 0 && !page.includes('/')) {
