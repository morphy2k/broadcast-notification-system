...
setInterval(() => dashboard.stats.push(), 6000);
...
// check if JWT outated
function ifJwtOutated() {
    let expDate = new Date(db.settings.get('auth.expiration_date').value()),
        date = new Date();

    if (expDate > date) {
        authentication.jsonwebtoken.generate();
    }
}
ifJwtOutated();
setInterval(() => ifJwtOutated(), 1800000);
