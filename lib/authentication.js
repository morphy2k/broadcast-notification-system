...
class Authentication extends JSONWebToken {
    ...
    expressHandler() {
        return (req, res, next) => {
            ...
            if (!mail.address &&
                ...
                !path.startsWith(`${authPath}/ressources`)) {
                return res.redirect(`${authPath}/signin`);
            }
            ...
            if (req.query.token) {
                ...
                return res.status(401).render('authentication/locked');
            }

            super.verify(token, (valid) => {
                if (!valid) {
                    return res.status(401).render('authentication/locked');
                } else {
                    if (req.query.token) {
                        ...
                        return res.cookie('jwt', token, cookieOptions).redirect('/dashboard');
                    }
                    return next();
                }
            });
    ...
    socketHandler(cookieParser) {
        return (socket, next) => {

            let cookies = socket.request.headers.cookie,
                jwt = qs.parse(cookies, ';', '=').jwt,
                token = cookieParser.signedCookie(jwt, this.secret);
